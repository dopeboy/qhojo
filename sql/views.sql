...
FROM BASE_VW b 
WHERE b.STATE_B_ID in (250,300,400);
...
LEFT JOIN RESPONSE_TIME_BASE2_VW b on a.TRANSACTION_ID=b.TRANSACTION_ID
GROUP BY LENDER_ID
ORDER BY NULL;
...
CREATE OR REPLACE VIEW NOTIFICATION_VW AS
SELECT
n.RECEIPIENT_USER_ID,
nt.DESCRIPTION as "NOTIFICATION_TYPE_DESC",
nt.TITLE as "NOTIFICATION_TITLE",
REPLACE(REPLACE(nt.TEXT_PATTERN,'%U',u1.FIRST_NAME),'%I',IF(CHAR_LENGTH(i.TITLE) > 21, CONCAT(LEFT(i.TITLE,21), '...'), i.TITLE)) as "NOTIFICATION_TEXT",
REPLACE(nt.LINK,'%T',n.TRANSACTION_ID) as "NOTIFICATION_LINK"
FROM NOTIFICATION n
INNER JOIN USER_VW u1 on n.SENDER_USER_ID=u1.USER_ID and u1.ACTIVE=1
INNER JOIN USER_VW u2 on n.RECEIPIENT_USER_ID=u2.USER_ID and u2.ACTIVE=1
INNER JOIN TRANSACTION t on n.TRANSACTION_ID = t.ID
INNER JOIN ITEM_VW i on t.ITEM_ID = i.ITEM_ID and i.ACTIVE=1
INNER JOIN NOTIFICATION_TYPE nt on n.TYPE_ID=nt.ID and nt.ACTIVE=1
WHERE n.UNREAD = 1
ORDER BY n.DATE ASC;
...
CREATE OR REPLACE VIEW BRAND_VW AS
SELECT
ID as "BRAND_ID",
NAME
FROM BRAND
...
CONCAT(b.NAME,' ',p.NAME,' ',c.NAME) as "FULL_NAME",
p.DESCRIPTION,
p.VALUE,
p.RATE,
p.URL,
...
p.DATE_CREATED
FROM PRODUCT p
INNER JOIN CATEGORY_VW c on p.CATEGORY_ID=c.CATEGORY_ID
INNER JOIN BRAND_VW b on p.BRAND_ID=b.BRAND_ID
WHERE p.ACTIVE=1;

CREATE OR REPLACE VIEW PRODUCT_IMAGE_VW AS
SELECT
...
INNER JOIN PRODUCT_VW p on p.PRODUCT_ID=pi.PRODUCT_ID
WHERE pi.ACTIVE=1;
