...
-- Show newest requests first; hence the sort by max(entry_date)
...
ORDER BY s.END_STATE_DATE DESC, a.TRANSACTION_ID, a.STATE_A_ID;

CREATE OR REPLACE VIEW RESERVED_AND_EXCHANGED_BASE_VW AS
SELECT 
TRANSACTION_ID,
max(ENTRY_DATE) as "END_STATE_DATE",
MAX(STATE_B_ID)
FROM BASE_VW 
GROUP BY TRANSACTION_ID 
HAVING MAX(STATE_B_ID) in (300,500)
ORDER BY max(ENTRY_DATE) DESC;

CREATE OR REPLACE VIEW RESERVED_AND_EXCHANGED_VW AS
SELECT 
s.*
FROM RESERVED_AND_EXCHANGED_BASE_VW a 
inner join BASE_VW s on s.TRANSACTION_ID=a.TRANSACTION_ID
ORDER BY a.END_STATE_DATE DESC, s.TRANSACTION_ID, s.STATE_A_ID;
...
-- on a.TRANSACTION_ID=s.TRANSACTION_ID
-- ORDER BY a.TRANSACTION_ID, a.STATE_A_ID;

CREATE OR REPLACE VIEW RETURNED_AND_REVIEWED_BASE_VW AS
SELECT 
TRANSACTION_ID,
max(ENTRY_DATE) as "END_STATE_DATE",
MAX(STATE_B_ID)
FROM BASE_VW 
GROUP BY TRANSACTION_ID 
HAVING MAX(STATE_B_ID) in (700,900,901)
ORDER BY max(ENTRY_DATE) DESC;

CREATE OR REPLACE VIEW RETURNED_AND_REVIEWED_VW AS
SELECT 
s.*
FROM RETURNED_AND_REVIEWED_BASE_VW a 
inner join BASE_VW s on s.TRANSACTION_ID=a.TRANSACTION_ID
ORDER BY a.END_STATE_DATE DESC, s.TRANSACTION_ID, s.STATE_A_ID;

-- CREATE OR REPLACE VIEW REVIEWED_BASE_VW AS
-- SELECT 
-- TRANSACTION_ID, 
-- MAX(STATE_B_ID) 
-- FROM BASE_VW 
-- GROUP BY TRANSACTION_ID 
-- HAVING MAX(STATE_B_ID) in (900,901);
-- 
-- CREATE OR REPLACE VIEW REVIEWED_VW AS
-- SELECT 
-- a.*
-- FROM BASE_VW a 
-- inner join REVIEWED_BASE_VW s 
-- on a.TRANSACTION_ID=s.TRANSACTION_ID
-- ORDER BY a.TRANSACTION_ID, a.STATE_A_ID;
...
CREATE OR REPLACE VIEW COMPLETED_TRANSACTION_CNT_VW AS
SELECT
...
FROM USER u
LEFT JOIN COMPLETED_TRANSACTION_CNT_VW ctc on u.ID=ctc.USER_ID
...
r.DATA,
case
    when STATE_B_ID=900 then r.LENDER_ID
    when STATE_B_ID=901 then r.BORROWER_ID
end as "REVIEWER_ID",
case
    when STATE_B_ID=900 then r.BORROWER_ID
    when STATE_B_ID=901 then r.LENDER_ID
end as "REVIEWEE_ID",
case
    when STATE_B_ID=900 then u1.NAME
    when STATE_B_ID=901 then u2.NAME
end as "REVIEWER_NAME",
case
    when STATE_B_ID=900 then u1.ACTIVE
    when STATE_B_ID=901 then u2.ACTIVE
end as "REVIEWER_ACTIVE",
case
    when STATE_B_ID=900 then u1.PROFILE_PICTURE_FILENAME
    when STATE_B_ID=901 then u2.PROFILE_PICTURE_FILENAME
end as "REVIEWER_PROFILE_PICTURE",
r.ENTRY_DATE as "REVIEW_DATE",
case
    when STATE_B_ID=900 then 'L'
    when STATE_B_ID=901 then 'B'
end as "SOURCE"
FROM REVIEWED_VW r
INNER JOIN USER_VW u1 on r.LENDER_ID=u1.USER_ID
INNER JOIN USER_VW u2 on r.BORROWER_ID=u2.USER_ID
WHERE STATE_B_ID in (900,901)
...
LEFT JOIN COMPLETED_TRANSACTION_CNT_VW ctc on i.LENDER_ID=ctc.USER_ID
