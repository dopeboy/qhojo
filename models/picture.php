...
{    
    public function uploadItemPictures($method,$item_id, $user_id)
    {
        ...
        if ($item_model->canUserModifyItem($item_id, $user_id) == false)
                throw new UserCannotModifyItemException($method, $user_id);
        
        global $item_picture_path;        
        $picture = new UploadHandler(
                array(
                        "upload_dir" => dirname(dirname(__FILE__)) . $item_picture_path . $item_id . '/', 
                        "upload_url" => $item_picture_path . $item_id . '/', 
                        "entity_id" => $item_id, 
                        "is_user" => 0,
...
class UploadHandler
{
    ...
    protected function set_additional_file_properties($file) {
        $file->deleteUrl = $this->options['script_url'] . 'picture/delete/' . $this->options['entity_id'] . '/' . $this->options['is_user'] 
            .$this->get_query_separator($this->options['script_url'])
            .$this->get_singular_param_name()
            .'='.rawurlencode($this->options['upload_url'].$file->name);
